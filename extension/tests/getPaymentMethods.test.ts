...
describe('getPaymentMethods unit tests', () => {
  ...
  it('Should return correctly formated response with saved request and response', async () => {
    ...
    const mockedMollieResponse = await getPaymentMethods(
      mockedRequest,
      mollieClient
    );
    ...
    expect(
      JSON.parse(addInterfaceInteractionObject.fields.request)
    ).toMatchObject(mockedPaymentMethodsRequest);
    expect(JSON.parse(setCustomFieldObject.value)).toMatchObject(
      mockedResponse
    );
  });
